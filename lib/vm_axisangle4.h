...
public:
     ...
     * @since Java 3D 1.2
     */
    AxisAngle4(const Vector3T& axis, T ang):
     ...
     * @since Java 3D 1.2
     */
    void set(const Vector3T& axis, T ang) {
        x = axis.x;
        y = axis.y;
        z = axis.z;
        angle = ang;
...
public:
    ...
    bool epsilonEquals(const AxisAngle4& a1, T epsilon) const {
        return (UtilT::abs(a1.x - x) <= epsilon) &&
            (UtilT::abs(a1.y - y) <= epsilon) &&
            (UtilT::abs(a1.z - z) <= epsilon) &&
            (UtilT::abs(a1.angle - angle) <= epsilon);
    }
...
TEMPLATE
void AxisAngle4T::set(const Quat4T& q1) {
    setFromQuat(q1.x, q1.y, q1.z, q1.w);
}

TEMPLATE
void AxisAngle4T::setFromMat(T m00, T m01, T m02,
                               T m10, T m11, T m12,
                               T m20, T m21, T m22) {
	...
	y = m02 - m20;
	z = m10 - m01;
	T sin = 0.5*UtilT::sqrt(x*x + y*y + z*z);
	angle = UtilT::atan2(sin, cos);
...
TEMPLATE
void AxisAngle4T::setFromQuat(T x, T y, T z, T w) {
	...
	T sin_a2 = UtilT::sqrt(x*x + y*y + z*z);  // |sin a/2|, w = cos a/2
	this->angle = 2.0*UtilT::atan2(sin_a2, w); // 0 <= angle <= PI , because 0 < sin_a2
	this->x = x;
	this->y = y;
	this->z = z;
}
...
#ifdef USE_TOSTRING
TEMPLATE 
STD::string AxisAngle4T::toString() const {
    VM_TOSTRING
}
#endif
...
#ifdef USE_IO
TEMPLATE
STD::ostream& operator<<(STD::ostream& o, const NAMESPACE::AxisAngle4T& a1) {
    return o << "(" << a1.x << "," << a1.y << ","
             << a1.z  << "," << a1.angle << ")";
}
#endif
