...
public:
     ...
     * @since Java3D 1.2
     */
    Point4(const Tuple3T& t1): Tuple4T(t1.x, t1.y, t1.z, 1) { }
    ...
    T distanceL1(const Point4& p1) const {
	    ...
	    + UtilT::abs(this->z-p1.z) + UtilT::abs(this->w-p1.w);
    }
    ...
    T distanceLinf(const Point4& p1) const {
        return UtilT::max(UtilT::abs(this->x-p1.x), UtilT::abs(this->y-p1.y), UtilT::abs(this->z-p1.z), UtilT::abs(this->w-p1.w));
    }
    ...
    Point4& operator=(const Tuple4T& t) {
        Tuple4T::operator=(t);
        return *this;
    }
};
...
TEMPLATE inline
NAMESPACE::Point4T operator*(T s, const NAMESPACE::Point4T& t1) {
    return operator*(s, (const NAMESPACE::Tuple4T&)t1);
}

#ifdef USE_IO
TEMPLATE inline
STD::ostream& operator<<(STD::ostream& o, const NAMESPACE::Point4T& t1) {
...
#else
typedef Point4 Point4d;
typedef Point4 Point4f;
#endif
